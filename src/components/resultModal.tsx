												...
												<div key={year} className="bg-gray-50 p-4 rounded-lg">
													<div className="flex justify-between items-center mb-2">
														...
														<span className="font-semibold text-yellow-800">
															GPA:{" "}
															{(
																fullSummary.totalGPA /
																fullSummary.totalCompleted
															).toFixed(2)}{" "}
															| Credits: {fullSummary.totalCredits}
														</span>
													</div>
													<div className="space-y-2">
														{Object.entries(fullSummary.sems).map(
															([sem, summary]) => (
																<div
																	key={sem}
																...
																>
																	...
																	<span className="text-gray-600">
																		GPA: {summary.gpa.toFixed(2)} | Credits:{" "}
...
const resultReview = (gpa: number) => {
	...
	if (gpa >= 2.0)
		return "You're meeting the minimum requirements, but consider seeking additional support to improve your grades.";
